...
- Lint warning baseline: 251 warnings (intentional debt; max-warnings=251 prevents unbounded growth)
...
- **Phase 1G (Final Hook Registration Split)**: Split hooks-registration.ts from 423 LOC → 134 LOC. Created `hooks/event-hooks.ts` (125 LOC), `hooks/tool-hooks.ts` (49 LOC), `hooks/dispose-hooks.ts` (82 LOC). All new files <200 LOC. Replaced getter-based state with plain sessionState object. Removed ~20 unused imports. Warnings dropped from 271 → 251. All checks green.
...
- ESLint v9 flat config with `max-warnings=251` for src lint (errors must be fixed before passing; warnings at 251 baseline)
...
2. Tighten ESLint rules for warnings (unused vars, any types, console in production code)
...
- ✅ hooks/event-hooks.ts: 125 LOC (under 200)
- ✅ hooks/tool-hooks.ts: 49 LOC (under 200)
- ✅ hooks/dispose-hooks.ts: 82 LOC (under 200)
...
## Current Lint Status
- `npm run lint:src`: 0 errors, 251 warnings → exits 0
