...
- Runtime-built fake-secret tests exercise SQLite-backed memory/session/event/project persistence, context cache, bridge journal, experience packets, all three distillation entry paths, AgentBook storage, and `AGENTBOOK_STATE.md`, with raw-row inspection as a backstop. Provider-boundary spies prove search and both embedding backfills receive protected text. Project/session identifiers remain operational, file lookup uses collision-resistant digest matching plus exact legacy compatibility, one explicit low-level opt-out remains honored, and existing source rows are treated as non-retroactive upgrade data.
...
- Design and ship a preview-first report/scrubber for legacy rows created before persistence-boundary redaction; never rewrite customer history automatically.
- Extend the supported redaction boundary and end-to-end proof to goal content and structured checkpoint/work-journal reference arrays.
