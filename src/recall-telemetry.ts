...
export async function initializeRecallTelemetrySchema(
  ...
  if (d === 'pg') {
    ...
    `);
    // Migrate existing table: make memory_id nullable + expand source CHECK
    await pool.query(`ALTER TABLE memory_recall_events ALTER COLUMN memory_id DROP NOT NULL`).catch(() => {});
    await pool.query(`
      DO $$
      BEGIN
        IF EXISTS (
          SELECT 1 FROM information_schema.table_constraints
          WHERE table_name = 'memory_recall_events' AND constraint_name = 'memory_recall_events_source_check'
        ) THEN
          ALTER TABLE memory_recall_events DROP CONSTRAINT memory_recall_events_source_check;
        END IF;
      END $$;
    `).catch(() => {});
    await pool.query(`
      ALTER TABLE memory_recall_events
      ADD CONSTRAINT memory_recall_events_source_check
      CHECK (source IN ('search', 'list', 'context_recall', 'graph', 'vector_only', 'text_only', 'text_fallback', 'empty_result'))
    `).catch(() => {});
  } else {
    // SQLite: table created by schema/sqlite/index.ts with expanded enum
  }
